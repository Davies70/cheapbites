  ...
  return (
    <div className="mt-12 relative">
      ...
      <ScrollArea className="w-full whitespace-nowrap rounded-md border custom-scrollbar">
        {/* Added snap-x and snap-mandatory here */}
        <div
          ref={scrollRef}
          className="flex w-max space-x-4 p-4 snap-x snap-mandatory"
        >
          {trendingPlaces.map((place) => (
            <div
              key={place.fsq_place_id}
              className="w-[85vw] max-w-[280px] sm:w-72 flex-shrink-0 snap-center" // Added snap-center and responsive width
            >
              <TrendingPlaceCard
                name={place.name}
